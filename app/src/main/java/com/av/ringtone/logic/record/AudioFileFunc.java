...
public class AudioFileFunc {
    ...
    public static String getAMRFilePath() {
        ...
        if (isSdcardExit()) {
            String fileBasePath = Environment.getExternalStorageDirectory().getAbsolutePath();
            mAudioAMRPath = fileBasePath + "/mp3 cut/";
            if (!new File(mAudioAMRPath).exists() ){
                new File(mAudioAMRPath).mkdirs();
            }
        }
        return mAudioAMRPath+createtFileName();
    }

    public static String getAMRFilePath(Activity activity) {
        String mAudioAMRPath = "";
        if (isSdcardExit()) {
            String fileBasePath = activity.getFilesDir().getPath();
            mAudioAMRPath = fileBasePath + "/mp3 cut/";
            if (!new File(mAudioAMRPath).exists() ){
                new File(mAudioAMRPath).mkdirs();
            }
